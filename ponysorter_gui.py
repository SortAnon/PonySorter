...
import time
import gc
from PySide2.QtWidgets import QApplication, QMainWindow, QDialog, QListWidgetItem, QDialogButtonBox, QDialogButtonBox, QMessageBox, QInputDialog
...
class LoadDialog(QDialog):
	...
	def detox_filenames(self):
		...
		for r, _, f in os.walk(path):
			for file in f:
				os.rename(os.path.join(r, file), os.path.join(r, file.replace(" ", "_")))
...
class MainWindow(QMainWindow):
	...
	moods = import_export.schema["properties"]["labels"]["items"]["properties"]["mood"]["items"]["enum"]
	...
	def load_episode(self):
		if self.unsavedChanges:
			changesAsk = QMessageBox.question(self, "Save changes?", "Save changes before loading new episode?", 
	...
	def add_audio_path(self):
		new_paths, okay = QInputDialog.getMultiLineText(self, "Add audio path(s)", 
	...
	def scan_for_files(self):
		...
		hash_thread = threading.Thread(target=import_export.hash_audio_files, args=(self.queue,))
